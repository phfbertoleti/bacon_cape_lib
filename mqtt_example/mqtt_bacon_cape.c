...
void publish(MQTTClient client, char* topic, char* payload) {
    ...
    MQTTClient_waitForCompletion(client, token, 1000L);    if (strcmp(MQTT_SUBSCRIBE_TOPIC_DISPLAY, topicName) == 0)
    {
        display7seg.alfanum_to_write = payload[0]-0x30;
        write_alfanum_display7seg(&display7seg);
    }
